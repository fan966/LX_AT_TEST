...
def http_api_web_test(interface_name,server_host = Config().ag_api_host,**replace_params):
    '''
    发送api请求
...
def get_web_login_id(http_res):
    import re
    res = http_res.headers
    p = re.compile('LoginSessionID=.*;', re.I)
    LoginSessionID = p.findall(res['Set-Cookie'])[0].replace(';', '')
    LoginSessionID = LoginSessionID.split('=')

    return LoginSessionID[1]
